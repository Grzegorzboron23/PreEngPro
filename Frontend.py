...
def FrontInit():
    ...
    ObjConfig3Btn = tkinter.Button(ObjConfigFrame, text="Ping", width = 10, command=lambda : Commands.CMD_PING(0,0))
...
def FrontSetPort(port):
    global ObjConfigLabel
    if(type(port) == str):
        ObjConfigLabel.config(text = "Open port: " + port)
    else:
        ObjConfigLabel.config(text="Port is closed")
...
def PortWindow(): #button to be set inactive
    ...
    ComboBox = ttk.Combobox(FramePort, values = Ports)
    ComboBox.pack()
    ...
    tkinter.Button(FrameBtns, text="Apply",width=10,padx=10).grid(row=0, column=1, padx=14)
...
def PortWindowApply(SelectedBoudrate,SelectedPort):
    pass
